...
from utils.platform.tx.device import Device, get_cached_device, save_device
...
async def get_qimei(version: str) -> QimeiResult:
    ...
    try:
        ...
        params = base64.b64encode(
            aes_encrypt(crypt_key.encode(), json.dumps(payload).encode())
        ).decode()
        extra = '{"appKey":"' + APP_KEY + '"}'
        sign = create(key, params, str(ts * 1000), nonce, SECRET, extra)
        res = await http.send_http_request(
