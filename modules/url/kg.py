import time
import random
from modules.constants import translateStrOrInt
from server.models.music import UrlResponse
from server.config import config
from server.exceptions import FailedException
from modules.info.kg import getMusicInfo
from utils.platform.kg import tools, getKey, sign_request


async def getUrl(songId: str, quality: str) -> UrlResponse:
    songId = songId.lower()
    info, info_body = await getMusicInfo(songId)
    album_id = info.albumId
    album_audio_id = info.kg.albumAudioId

    user_info = random.choice(config.get("modules.platform.kg.users"))

    params = {
        "album_id": album_id,
        "userid": user_info["userid"],
        "area_code": 1,
        "hash": songId,
        "mid": tools["mid"],
        "appid": "1005",
        "ssa_flag": "is_fromtrack",
        "clientver": "20349",
        "token": user_info["token"],
        "album_audio_id": album_audio_id,
        "behavior": "play",
        "clienttime": int(time.time()),
        "pid": "2",
        "key": getKey(songId, user_info),
        "quality": tools["qualityMap"][quality],
        "version": "20349",
        "dfid": "-",
        "pidversion": 3001,
    }

    headers = {
        "User-Agent": "Android12-AndroidCar-20089-46-0-NetMusic-wifi",
        "KG-THash": "255d751",
        "KG-Rec": "1",
        "KG-RC": "1",
    }

    req = await sign_request(
        "http://tracker.kugou.com/v5/url",
        params,
        {"headers": headers},
    )
    body = dict(req.json())

    if not body.get("url"):
        raise FailedException(translateStrOrInt(body["status"]))

    play_url = body["url"][0]

    return UrlResponse(url=play_url, quality=quality)